...
def main(args: DictConfig):
    ...
    processed_pdb_id_list = []
    # iterate through all the hdf5 files
    for hdf5_file in hdf5_file_path.iterdir():
        pdb_id = hdf5_file.name.split("_")[0]

        # skip processed pdb id
        if pdb_id in processed_pdb_id_list:
            continue
        ...
        f = h5py.File(selected_pdb_hdf5_file, "a")
        box_hashmap = {}
        idx = 0
        for chain in f.keys():
            num_boxes = 0
            for box in tqdm(f[chain], total=len(list(f[chain].keys()))):
        ...
        f.close()
        processed_pdb_id_list.append(pdb_id)
